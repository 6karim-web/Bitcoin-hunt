package com.satoshi.benchmark
  ...
  class MainActivity : AppCompatActivity() {
      ...
      private fun startBenchmark() {
          ...
          val count = targetHashSet.size + if (manualTarget.isNotEmpty()) 1 else 0
          appendConsole("[TARGET] Checking " + count + " address(es)")
          appendConsole("----------------------------------------")
          ...
          while (isRunning.get() && !Thread.currentThread().isInterrupted) {
              try {
                  ...
                  if (localCount % logInterval == 0L) {
                      ...
                      mainHandler.post { appendConsole("[SAMPLE] " + snippet + " | wif=" + wif.take(8) + "...") }
                  }
                  ...
                  if (inHashSet || matchesManual) {
                      ...
                      val source = if (inHashSet && matchesManual) "HashSet + Manual"
                                   else if (inHashSet) "targets.txt"
                                   else "Manual Target"
                      isRunning.set(false)
